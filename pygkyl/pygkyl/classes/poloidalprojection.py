...
class PoloidalProjection:
  def __init__(self):
    ...
    self.bcPhaseShift = 0
    self.kxIntC = 0
    self.field_kintPos = None
    self.z_ex = None
    self.z_int = None
    self.field_kint = None
    self.nxIntC = 0
    self.zNumInt = 0
    self.field_RZ = None
    self.xyz2RZ = None
  ...
  def setup(self, simulation, fieldName='', timeFrame=0, nzInterp=32):
    '''
    ...
    field_frame = Frame(simulation, name=fieldName, tf=timeFrame, load=True)
    ...
    field_kex = np.zeros(kxIntC+np.array([0,0,2]), dtype=np.cdouble)
    ...
    field_kintPos = np.zeros((kxIntC[0],kxIntC[1],zNumInt), dtype=np.cdouble)

    #.Append negative ky values.
    field_kint = np.zeros((kxIntC[0],2*kxIntC[1],zNumInt), dtype=np.cdouble)
    ...
    field_RZ = np.zeros([nxIntC[0],zNumInt])
    ...
    self.xi_lcfs = xi_lcfs
    self.field_kex = field_kex
    self.bcPhaseShift = bcPhaseShift
    self.kxIntC = kxIntC
    self.field_kintPos = field_kintPos
    self.z_ex = z_ex
    self.z_int = z_int
    self.field_kint = field_kint
    self.nxIntC = nxIntC
    self.zNumInt = zNumInt
    self.field_RZ = field_RZ
    self.xyz2RZ = xyz2RZ
    self.RIntN = RIntN
    self.ZIntN = ZIntN
    self.RInt_lcfs = RInt_lcfs
    self.ZInt_lcfs = ZInt_lcfs
    self.Z_axis = Z_axis

  def plot(self, timeFrame=0, outFilename='', colorMap = 'inferno', doInset=True, scaleFac=1., 
           xlim=[],ylim=[],clim=[],climSOL=[]):
    

    field_frame = Frame(self.simulation, self.fieldName, timeFrame, load=True)
    dataInterp = field_frame.values
    field = np.squeeze(dataInterp)
    ...
    #.Extend along z by in each direction by applying twist-shift BCs in the 
    #.closed-flux region, and just copying the last values (along z) in the SOL.
    self.field_kex[:,:,1:-1] = field_ky
    for j in range(self.kxIntC[1]):
        self.field_kex[:self.xi_lcfs,j,0]  = field_ky[:self.xi_lcfs,j,-1]*np.exp( self.bcPhaseShift*j)
        self.field_kex[:self.xi_lcfs,j,-1] = field_ky[:self.xi_lcfs,j, 0]*np.exp(-self.bcPhaseShift*j)
        self.field_kex[self.xi_lcfs:,j,0]  = field_ky[self.xi_lcfs:,j, 1]
        self.field_kex[self.xi_lcfs:,j,-1] = field_ky[self.xi_lcfs:,j,-2]

    #.Interpolate onto a finer mesh along z.
    for i in range(self.kxIntC[0]):
        for j in range(self.kxIntC[1]):
            self.field_kintPos[i,j,:] = pchip_interpolate(self.z_ex, self.field_kex[i,j,:], self.z_int)

    #.Append negative ky values.
    for i in range(self.kxIntC[0]):
        for j in range(self.kxIntC[1]):
            self.field_kint[i,j,:]  = self.field_kintPos[i,j,:]
            self.field_kint[i,-j,:] = np.conj(self.field_kintPos[i,j,:])

    #.Convert (x,y,z) data to (R,Z):
    for i in range(self.nxIntC[0]):
        for k in range(self.zNumInt):
            self.field_RZ[i,k] = np.real(np.sum(self.xyz2RZ[i,:,k]*self.field_kint[i,:,k]))

    # handle colormap limits
    vlims, vlims_SOL = data_utils.get_minmax_values(self.simulation, self.fieldName, [timeFrame])
    if colorMap == 'inferno': 
    ...
    hpl1a = list()
    pcm1 = ax1a[0].pcolormesh(self.RIntN, self.ZIntN, np.squeeze(self.field_RZ)/scaleFac, shading='auto',cmap=colorMap,
                              vmin=fldMin,vmax=fldMax)
    hpl1a.append(pcm1)

    #fig1a.suptitle
    ax1a[0].set_title('t = %.2f'%(field_frame.time)+' '+self.simulation.normalization.dict['tunits'],fontsize=titleFontSize) 
    ...
    yWidth = 0.01
    yCorner = self.Z_axis - 0.5*yWidth
    ...
    if doInset: 
      ...
      img_in = axins2.pcolormesh(self.RIntN, self.ZIntN, np.squeeze(self.field_RZ)/scaleFac, 
      ...
      x1, x2, y1, y2 = 1.07, 1.16, self.Z_axis-.1, self.Z_axis+.1
