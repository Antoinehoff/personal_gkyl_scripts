...
small velocity box size. It is a 2D reduction of the 3D baseline of TCV NT.
*/
#include <math.h>
#include <stdio.h>
#include <time.h>
...
struct gk_app_ctx {
  ...
  // Initial conditions.
  double k_Te; // normalized temperature gradient (R/L_Te)
  double k_Ti; // normalized temperature gradient (R/L_Ti)
  double k_n;  // normalized density gradient (R/L_n)
...
void density_init(double t, const double * GKYL_RESTRICT xn, double* GKYL_RESTRICT fout, void *ctx)
{
  ...
  double k_n = app->k_n;
  double R = app->R_axis;
  // Linear initial density profile with a small perturbation.
  fout[0] = n0*(1.0 - k_n*x/R) + n0*0.01*sin(8.*M_PI*x/R);
...
void temp_elc(double t, const double * GKYL_RESTRICT xn, double* GKYL_RESTRICT fout, void *ctx)
{
  ...
  struct gk_app_ctx *app = ctx;
  double Te0 = app->Te0;
  double k_Te = app->k_Te;
  double R = app->R_axis;
  double Ly = app->Ly;
  // Linear initial temperature profile with a small perturbation.
  fout[0] = Te0*(1.0 - k_Te*x/R) + Te0*0.001*sin(8.*M_PI*y/Ly);
}
...
void temp_ion(double t, const double * GKYL_RESTRICT xn, double* GKYL_RESTRICT fout, void *ctx)
{
  ...
  struct gk_app_ctx *app = ctx;
  double Ti0 = app->Ti0;
  double k_Ti = app->k_Ti;
  double R = app->R_axis;
  double Ly = app->Ly;
  // Linear initial temperature profile with a small perturbation.
  fout[0] = Ti0*(1.0 - k_Ti*x/R) + Ti0*0.001*sin(8.*M_PI*y/Ly);
}
...
create_ctx(void)
{
  ...
  double Te0 = 500*eV;
  double Ti0 = 500*eV;
  double n0  = 2.0e20;   // [1/m^3]
  double k_Te = 0.0; // normalized temperature gradient (R/L_Te)
  double k_Ti = 7.0; // normalized temperature gradient (R/L_Ti)
  double k_n  = 2.2;  // normalized density gradient (R/L_n)
  double Bref = 1.4;   // Reference magnetic field [T].
...
main(int argc, char **argv)
{
  ...
  struct gkyl_gyrokinetic_field field = {
    .gkfield_id = GKYL_GK_FIELD_ES_TS,
    .xLCFS = ctx.x_LCFS,
