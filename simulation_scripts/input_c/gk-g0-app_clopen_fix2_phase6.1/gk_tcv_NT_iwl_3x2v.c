/*
This input file sets up a 3x2v GK simulation with Gkeyll with an inner wall limited configuration in a
domain containing both closed and opened field lines. The physical parameters are taken from a NT
TCV discharge (courtesy of T. Bernard). The resolution is 48x16x12x6 but can be lowered down up to 24x12x12x10x6.
This input file can run with the gk-g0-app_clopen_fix2_phase6.1 branch, commit: 986badfbcabc40c8bbafdcc9ff095eee3cdaa9dc
*/
#include <math.h>
#include <stdio.h>
#include <time.h>
...
struct gk_app_ctx {
  ...
  // Source parameters.
  double n_src;        // Amplitude of the OMP source
  double x_src;        // Radial location of the OMP source.
  double Te_src;       // Te for the OMP source.
  double Ti_src;       // Ti for the OMP source.
  double sigma_src;    // Radial spread of the OMP source.
...
void density_src(double t, const double * GKYL_RESTRICT xn, double* GKYL_RESTRICT fout, void *ctx)
{
  ...
  double x_src = app->x_src;
  double sigma_src = app->sigma_src;
  double floor_src = app->floor_src;

  fout[0] = n_src*(exp(-(pow(x-x_src,2))/(2.*pow(sigma_src,2)))+floor_src);
}
...
create_ctx(void)
{
  ...
  double Te0 = 100*eV;   // Reference electron temperature [eV].
  double Ti0 = 100*eV;   // Reference ion temperature [eV].
  ...
  double rho_s = c_s/omega_ci; // Sound gyroradius.
